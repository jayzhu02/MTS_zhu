...
def generate(model, spectrogram, batched, target, overlap, save_str=None):
    ...
    if mel.ndim != 2 or mel.shape[0] != hp.num_mels:
        raise ValueError(r'Expected a numpy array shaped (n_mels, n_hops), but got {wav.shape}!')
    _max = np.max(mel)
    _min = np.min(mel)
...
def gen_from_mel(model: WaveRNN, mel, save_path: Path, batched, target, overlap):
