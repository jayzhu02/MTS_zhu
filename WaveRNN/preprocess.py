...
def process_data(data_root, data_dirs, output_path):
    ...
    c = 1
    # modified to MT2S
    meta_file = os.path.join(data_root, "train.txt")
    with open(meta_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            file_id = os.path.splitext(os.path.basename(item['audio']))[0]
            wav_d = os.path.join(data_root, item['audio'])
            wav = convert_file(os.path.join(wav_d))
            mel = np.load(os.path.join(data_root,  item['spectrogram']))
            mel = normalize(mel)
            ...
            dataset.append((file_id, mel.shape[-1], item['language']))
            c += 1
    ...
    print(f"Preprocessing done, total processed wav files: {c}")
